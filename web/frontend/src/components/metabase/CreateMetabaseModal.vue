  ...
  <BaseModal v-model="open" title="Nouvelle instance Metabase">
    ...
    <div class="form-group" style="margin-bottom:6px">
      ...
      <select v-model="form.linked_pg_id" style="width:100%">
        <option value="">— Aucune (H2 embarqué) —</option>
        <option v-for="pg in runningPgs" :key="pg.id" :value="pg.id">{{ pg.name }}</option>
      </select>
    </div>
    <p style="font-size:12px;color:#9CA3AF;margin-bottom:20px">Si une base est liée, Metabase stocke ses métadonnées dans PostgreSQL.</p>
...
import { apiGetPostgres, apiCreateMetabase } from '../../lib/api.js'
...
async function submit() {
  ...
  try {
    ...
    const res = await apiCreateMetabase(body)
    if (res && res.ok) {
      open.value = false
